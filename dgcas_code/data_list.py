...
#该函数是定义一个make_dataset函数，该函数返回一个列表，列表中的元素为元组，元组中的第一个元素为图片路径，第二个元素为图片的标签。
#image_list是图片路径列表，labels是标签列表
#检查labels是否被提供
#image_list是cub200_2011_multi.txt文件中的图片路径列表，labels没有提供
def make_dataset(image_list, labels):
    if labels:
        len_ = len(image_list)
        #strip(）去除字符串首尾的空格，labels[i, :]表示第i行，所有列,循环的次数为len_，即图片数量
        images = [(image_list[i].strip(), labels[i, :]) for i in range(len_)]
    #如果labels没有被提供，则根据图片路径列表中的图片名称，从图片名称中提取标签，标签为图片名称中的数字，例如，图片名称为1.jpg，则提取到的标签为1
    #split()函数是一个字符串分割函数，默认按照空格分割，返回一个列表
    #labels没有提供，image_list是cub200_2011_multi.txt文件中的图片路径列表，所以需要从图片路径列表中提取标签
    else:
        if len(image_list[0].split()) > 2:
            #image_list中第一个元素，提取出图片路径,剩余部分转化为整数列表，使用np.array()转化为Numpy数组
            images = [(val.split()[0], np.array([int(la) for la in val.split()[1:]])) for val in image_list]
        else:
            #如果只有两个部分，则第一部分为图片路径，第二部分为图片标签
...
def pil_loader(path):
    ...
    with open(path, 'rb') as f: #Rb 二进制模式打开
        with Image.open(f) as img:
            return img.convert('RGB') #返回一个RGB格式的图片
...
def default_loader(path):
    ...
    # else:
    return pil_loader(path) #返回一个RGB格式的图片
...
class ImageList(object):
    ...
    #python类的构造函数（__init__ 方法），用于加载和预处理图像数据
    #--int--是python类的构造函数，当创建类的实例时自动调用
    #self 是python中的关键字，代表当前实例，在类中，self代表当前实例，在类外，self代表类本身
    # imgs_list是图片路径和标签的列表
    #labels是标签列表,transform是数据增强，target_transform是标签增强，loader是加载图片，default_loader是默认加载图片函数
    #/dataset_list/cub200_2011_multi.txt是image_list的路径
    #transform是对原始数据进行预处理之后的结果
    def __init__(self, image_list, labels=None, transform=None, target_transform=None,
                 loader=default_loader):
        imgs = make_dataset(image_list, labels)  # 返回路径和标签
        if len(imgs) == 0:
            #如果imgs为空，则抛出异常
            raise (RuntimeError("Found 0 images in subfolders of: "))

        self.imgs = imgs  #存储图像路径和标签
        self.transform = transform #存储图像变换函数
        self.target_transform = target_transform #存储标签变换函数
        self.loader = loader #存储加载图片函数
    ...
    def __getitem__(self, index):
        ...
        """
        path, target = self.imgs[index] #获取索引为index的图片路径和标签
        img = self.loader(path)  # 加载图片RGB格式，还没有转换到三维
        if self.transform is not None:
            img = self.transform(img)
        if self.target_transform is not None:
            target = self.target_transform(target)

        return img, target  #从数据集中返回一个样本（图像和对应标签）


    #该函数 select_idx 用于查找给定图片路径 img_path 在 self.imgs 列表中的索引位置
...
class ImageList_tr_te(object):
    ...
    #python类的构造函数（__init__ 方法），dataset_path是数据集路径，split是指定数据集的分割方式，默认为train
    def __init__(self, image_list, labels=None, transform=None, target_transform=None,
                 loader=default_loader, dataset_path='', split='train'):
        ...
        #创建了一个image_file的变量，通过将字符串‘/images.txt’附加到dataset_path变量上来定义一个文件路径
        # 文件 images.txt 应该包含数据集中所有图像的文件名或相对路径，通常每行一个
        images_file = dataset_path + '/images.txt'
        label_file = dataset_path + '/image_class_labels.txt'
        attributes_file = dataset_path + '/attributes/image_attribute_labels.txt'
        class_attributes_file = dataset_path + '/attributes/class_attribute_labels_continuous.txt'
        split_file = dataset_path + '/train_test_split.txt'
        bb_file = dataset_path + '/bounding_boxes.txt'
        attribute_name_file = dataset_path + '/attributes.txt'
        processed_attribute_file = dataset_path + '/attributes/processed_attributes.txt'

        #读取一个分割文件split_file，根据文件内容确定图像属于训练集还是测试集
        with open(split_file) as split_rf:
            train_test_list = [] #用于存储每个图像的训练或测试标签
            train_idx = [] #用于存储属于训练集的图像索引
            test_idx = [] #用于存储属于测试集的图像索引
            i = 0
            for line in split_rf.readlines():
                strs = line.strip().split(' ')
                train_test_list.append(strs[1]) #将列表中第二个元素增加到train_test_list中
                if strs[1] == '1':
                    train_idx.append(i)
                else:
                    test_idx.append(i)
        ...
        # image list
        imgs = make_dataset(image_list, labels)  # 返回路径和细粒度标签
        imgs_train = []
        imgs_test = []
        if len(imgs) == 0:
            raise (RuntimeError("Found 0 images in subfolders of: "))
        for i, i_img in enumerate(imgs):
            if train_test_list[i] == '1':
                imgs_train.append(i_img)
            else:
                imgs_test.append(i_img)
...
# 每次都在新服务器重新执行这段代码即可，都会自动存储到 dataset_list文件夹下
if __name__ == '__main__':
    ...
    #############################################################################################
    #    将bd将图片提取出来，imagefolder可以直接用的那种
    #############################################################################################
    ...
    # #############################################################################################
    # # 将comcars web部分存成不含年份的，imagefolder可以直接用的那种
    # # 新服务器需要进行这一步的处理，只处理一次就行。
    # #############################################################################################
    ...
    # #############################################################################################
    # # 将comcars数据集的 dataset_list_yuan/ 文件夹里的路径给改了 运算
    # #############################################################################################
    ...
    #从多个文件中提取图像路径和类别信息，构建新的路径格式，然后将这些信息保存到新的文本文件中
        ...
        for i_domain_name, i_domain_path in file_path.items():
            ...
            with open(dataset_source) as image_file:
                ...
                if len(image_list[0].split()) > 2:
                    ...
                    for single_image in image_list:
                        e_all = single_image.split()
                        e_0 = single_image.split()[0]  # 获取文件名
                        e_1 = single_image.split()[1:]  # 获取各个粒度的类别
                        ...
                        e_2 = e_path[start_index:]  # cub_drawing 是3，cub_200_2011是5
                        root_path = '/data'
