...
class SetCriterion(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.b_part_oth = args.b_part_oth
        ...
        self.loss['loss_feat_oth'] = torch.tensor(0.0)
        self.loss['loss_part_oth'] = torch.tensor(0.0)
        self.loss['loss_same_sa_diff_g_com'] = torch.tensor(0.0)
        self.loss['loss_same_sa_diff_g_pvt'] = torch.tensor(0.0)
        self.loss['loss_diff_sa_same_g_com'] = torch.tensor(0.0)
        self.loss['loss_diff_sa_same_g_pvt'] = torch.tensor(0.0)
        self.loss['loss_diff_sa_diff_g_com'] = torch.tensor(0.0)
        self.loss['loss_diff_sa_diff_g_pvt'] = torch.tensor(0.0)

        self.loss['cnfd_loss'] = torch.tensor(0.0)
        self.loss['cndg_loss'] = torch.tensor(0.0)
        self.loss['loss_centriods'] = torch.tensor(0.0)
        self.oth_num['oth_fp'] = 0.0
        self.oth_num_T1['oth_fp_T1'] = 0.0
        self.oth_num_T2['oth_fp_T2'] = 0.0
        self.singular_dist['oth_fp'] = 0.0
        self.oth_num_part['oth_fp_part'] = 0.0

        for i_e_g in self.ele_g[::-1]:
            self.oth_num[f'oth_{i_e_g}'] = 0.0
            self.oth_num_T1[f'oth_T1_{i_e_g}'] = 0.0
            self.oth_num_T2[f'oth_T2_{i_e_g}'] = 0.0
            self.singular_dist[f'oth_{i_e_g}'] = 0.0
            self.oth_num_part[f'oth_{i_e_g}_part'] = 0.0
        ...
        self.w_oth_num['oth_w'] = 0.0
        self.w_oth_num_T1['oth_w_T1'] = 0.0
        self.w_oth_num_T2['oth_w_T2'] = 0.0
        if self.G > 1:
            for i_e_g in self.ele_g[::-1]:
                self.w_oth_num[f'oth_w_{i_e_g}'] = 0.0
                self.w_oth_num_T1[f'oth_w_T1_{i_e_g}'] = 0.0
                self.w_oth_num_T2[f'oth_w_T2_{i_e_g}'] = 0.0

        self.accu_acc = torch.zeros((1, self.G), dtype=torch.float64, device='cuda')
        self.accu_csi = torch.zeros((1), dtype=torch.float64, device='cuda')
        self.accu_labels = torch.zeros((1, self.g_dataset_lenth), dtype=torch.float64, device='cuda')
        self.Ele_g = list(range(self.G))
        for i_e_g in self.Ele_g[::-1]:
            self.centriods[f'feat_centroid_common_{i_e_g}'] = nn.Parameter(
                torch.zeros((self.cate_num_all[0], feat_part[0]), dtype=torch.float32, device='cuda'),
                requires_grad=False
            )
            self.centriods[f'feat_centroid_privac_{i_e_g}'] = nn.Parameter(
                torch.zeros((self.cate_num_all[0], feat_part[1] - feat_part[0]), dtype=torch.float32, device='cuda'),
                requires_grad=False
            )
            self.centriods[f'feat_centroid_noise_{i_e_g}'] = nn.Parameter(
                torch.zeros((self.cate_num_all[0], feat_part[2] - feat_part[1]), dtype=torch.float32, device='cuda'),
                requires_grad=False
            )
            self.centriods[f'feat_centroid_all_{i_e_g}'] = nn.Parameter(
                torch.zeros((self.cate_num_all[0], feat_part[2]), dtype=torch.float32, device='cuda'),
                requires_grad=False
            )
    ...
    def acc_compute(self, f_logits, c_logits, labels):
        num_allsamples = float(labels.size()[0]) #样本总数
        for i, (i_key, i_value) in enumerate(self.acc.items()):
            if i == 0:
                _, predict = torch.max(f_logits.data.float(), 1)  # 最大的那个位置
                ...
                _, predict = torch.max(c_logits[i - 1].data.float(), 1)  # 最大的那个位置
        ...
        acc_all = torch.tensor([v for v in self.acc.values()], dtype=torch.float32, device='cuda').unsqueeze(0)  # 顺序错误
    ...
    def csi_compute(self, f_logits, c_logits, labels):
        num_allsamples = float(labels.size()[0])
        m_labels_source = labels[:, :self.G]
        samples_consistency = 0
        pred_results_soft = []
        _, f_predict = torch.max(f_logits.data.float(), 1)
        pred_results_soft.append(f_predict)
        if c_logits is not None:
            for iii in range(len(c_logits)):
                _, c_predict = torch.max(c_logits[iii].data.float(), 1)
                pred_results_soft.append(c_predict)
        pred_results_soft = torch.stack(pred_results_soft).T
        for iii in range(pred_results_soft.shape[0]):
            i_sample_consistency = torch.sum(m_labels_source[iii] == pred_results_soft[iii]) / self.G
            samples_consistency += i_sample_consistency
            self.accu_csi = torch.cat((self.accu_csi, i_sample_consistency.view(1)), 0)
        return samples_consistency / num_allsamples, num_allsamples
    def output_consistency(self):
        num_allsamples = float(len(self.accu_csi) - 1)
        consistency = torch.sum(self.accu_csi[1:]) / num_allsamples
        self.accu_csi = torch.zeros((1), dtype=torch.float32, device='cuda')
        return consistency, num_allsamples
    def init_loss_and_oth(self):
        for key, value in self.oth_num.items():
            self.oth_num[key] = 0.0
        for key, value in self.oth_num_T1.items():
            self.oth_num_T1[key] = 0.0
        for key, value in self.oth_num_T2.items():
            self.oth_num_T2[key] = 0.0
        for key, value in self.w_oth_num.items():
            self.w_oth_num[key] = 0.0
        for key, value in self.w_oth_num_T1.items():
            self.w_oth_num_T1[key] = 0.0
        for key, value in self.w_oth_num_T2.items():
            self.w_oth_num_T2[key] = 0.0
        for key, value in self.oth_num_part.items():
            self.oth_num_part[key] = 0.0
        for key, value in self.singular_dist.items():
            self.singular_dist[key] = 0.0
    ...
    def output_all_centriods(self):
        return self.centriods

    def forward(self, labels_source, logits_fine, logits_coarse, logits_cnfd, domain_predicted, out_feat_btnk, iter_num,
                cond_w_all=None, w_weight=None,
                b_update_centroids=False, b_eva=False, prototype = False):
        ...
        fine_labels_onehot = torch.zeros(logits_fine_source.size()).scatter_(1, fine_labels_source_cpu, 1).to(
            self.device)
        ...
        if self.args.bn_cfdg is True:
            ...
            if self.args.b_part_oth is True:
                ...
                if self.args.b_part_oth is True:
                    for i_g in range(0, len(out_feat_btnk)):
                        ...
                        same_sa_same_g_feat_dis_all_part[i_g] = torch.norm(i_g_feat_part_proto_dis,
                                                                           p=2)
                    ...
                    if self.args.b_part_oth is True:
                        ...
                        pass
                self.oth_num_part['oth_fp_part'] += same_sa_same_g_feat_dis_all_part[0].item()
                if len(out_feat_btnk) > 1:
                    for i_e_g in self.ele_g[::-1]:
                        self.oth_num_part[f'oth_{i_e_g}_part'] += same_sa_same_g_feat_dis_all_part[self.G - i_e_g].item()
            else:
            ...
            if self.args.b_same_sa_diff_g_com is True:
                ...
                if self.args.b_same_sa_diff_g_com is True:
                    total_loss = total_loss - loss_same_sa_diff_g_com * lambda_l2 * self.args.c_same_sa_diff_g_com
                else:
                    pass
            else:
            ...
            if self.args.b_diff_sa_same_g_pvt is True:
                ...
                for i_g in range(self.G):
                    ...
                    g_feat_btnk = out_feat_btnk[i_g, :, feat_part[0]:feat_part[1], :]
                    ...
                    diff_sa_same_g_pvt_dis_all[i_g] = feat_sim_cos_T1(diff_sa_same_g_pvt_pttp_all,
                                                                      method='diag-I')
                loss_diff_sa_same_g_pvt = torch.sum(diff_sa_same_g_pvt_dis_all) / all_num_cat
                if self.args.b_diff_sa_same_g_pvt is True:
                    total_loss = total_loss + loss_diff_sa_same_g_pvt * stage2_alfar * self.args.c_diff_sa_same_g_pvt
                else:
                    pass
            else:
                ...
                for i_g in range(self.G - 1):
                    ...
                    for i_g_num_cat_father in range(g_num_cat_father):
                        ...
                        for i_g_son_cat in range(g_i_num_cat_son):
                            ...
                            g_i_feat_index_son = (g_lables_son == g_i_cat_son).nonzero(as_tuple=True)[
                                0]

                            g_i_son_cat_feat = torch.index_select(g_feat_btnk, 0, g_i_feat_index_son)
                            g_i_son_cat_feat_com = g_i_son_cat_feat[:, :feat_part[0], :]  # [5,128,49]
                            diff_sa_diff_g_com_pttp_all[i_g_son_cat] = torch.mean(
                                torch.mean(g_i_son_cat_feat_com, dim=2), dim=0)  # [256]
            ...
            if b_update_centroids is True:
                with torch.no_grad():
                    if isinstance(out_feat_btnk, list):
                        out_feat_btnk = torch.stack(out_feat_btnk[1:])
                    for i_g in range(self.G):
                        g_lables_sc = labels_source[:, self.G - 1 - i_g]
                        g_lables_sc_set = torch.unique(g_lables_sc, dim=0)
                        g_num_cat = len(g_lables_sc_set)
                        g_feat_common = out_feat_btnk[i_g, :, :feat_part[0], :]
                        g_feat_privacy = out_feat_btnk[i_g, :, feat_part[0]:feat_part[1], :]
                        g_feat_noise = out_feat_btnk[i_g, :, feat_part[1]:, :]
                        g_feat_all = out_feat_btnk[i_g, :, :, :]
                        for i_g_num_cat in range(g_num_cat):
                            g_i_cat = g_lables_sc_set[i_g_num_cat]
                            g_i_feat_index = (g_lables_sc == g_i_cat).nonzero(as_tuple=True)[0]
                            g_i_feat_btnk_common = torch.index_select(g_feat_common, 0, g_i_feat_index)
                            g_i_feat_btnk_privacy = torch.index_select(g_feat_privacy, 0, g_i_feat_index)
                            g_i_feat_btnk_noise = torch.index_select(g_feat_noise, 0, g_i_feat_index)
                            g_i_feat_btnk_all = torch.index_select(g_feat_all, 0, g_i_feat_index)

                            g_i_feat_btnk_common = torch.mean(torch.mean(g_i_feat_btnk_common, dim=2), dim=0)
                            g_i_feat_btnk_privacy = torch.mean(torch.mean(g_i_feat_btnk_privacy, dim=2), dim=0)
                            g_i_feat_btnk_noise = torch.mean(torch.mean(g_i_feat_btnk_noise, dim=2), dim=0)
                            g_i_feat_btnk_all = torch.mean(torch.mean(g_i_feat_btnk_all, dim=2), dim=0)
                            self.centriods[f'feat_centroid_common_{i_g}'][g_i_cat, :] = self.args.mu * \
                                                                                        self.centriods[
                                                                                            f'feat_centroid_common_{i_g}'][
                                                                                        g_i_cat,
                                                                                        :] + (
                                                                                                1 - self.args.mu) * g_i_feat_btnk_common
                            self.centriods[f'feat_centroid_privac_{i_g}'][g_i_cat, :] = self.args.mu * \
                                                                                        self.centriods[
                                                                                            f'feat_centroid_privac_{i_g}'][
                                                                                        g_i_cat,
                                                                                        :] + (
                                                                                                1 - self.args.mu) * g_i_feat_btnk_privacy
                            self.centriods[f'feat_centroid_all_{i_g}'][g_i_cat, :] = self.args.mu * self.centriods[
                                                                                                        f'feat_centroid_all_{i_g}'][
                                                                                                    g_i_cat, :] + (
                                                                                             1 - self.args.mu) * g_i_feat_btnk_all
                            self.centriods[f'feat_centroid_noise_{i_g}'][g_i_cat, :] = self.args.mu * \
                                                                                       self.centriods[
                                                                                           f'feat_centroid_noise_{i_g}'][
                                                                                       g_i_cat,
                                                                                       :] + (
                                                                                               1 - self.args.mu) * g_i_feat_btnk_noise

            self.loss['loss_part_oth'] += loss_part_oth.item()
        ...
        return total_loss, self.loss, self.oth_num, self.oth_num_T1, self.oth_num_T2, self.oth_num_part, \
            self.w_oth_num, self.w_oth_num_T1, self.w_oth_num_T2
