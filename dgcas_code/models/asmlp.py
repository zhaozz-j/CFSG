...
class AxialShiftedBlock(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        B, C, H, W = x.shape  # torch.Size([2, 96, 304, 200])
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        ...
        x = x.view(B, 4 * C, H//2, W//2)  # B H/2*W/2 4*C torch.Size([2, 384, 152, 100])
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, register_hook=False):
...
class ASMLP(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, 
                 ...
                 out_indices=[0, 1, 2, 3],
                 frozen_stages=-1,
                 use_checkpoint=False, logger=None, pretrained=None,**kwargs):
        super().__init__()
        self.logger = logger
        ...
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # 12个数，stochastic depth decay rule
        ...
        # add a norm layer for each output  这里和分类那个不一样
        ...
        self._freeze_stages()
        self.num_channels = num_features[-1]

        # self.conv = nn.Conv2d(in_channels=self.num_channels, out_channels=self.num_channels, kernel_size=1, stride=1)
        self.conv = nn.Conv2d(in_channels=self.num_channels, out_channels=2048, kernel_size=1, stride=1)  # ijcai
    ...
    def init_weights(self, pretrained):
        ...
        if isinstance(pretrained, str):
            ...
            self.logger.info('loaded bkb checkpoint from {}'.format(pretrained))

        elif pretrained is None:
            self.apply(_init_weights)
            self.logger.info('no loaded checkpoint and initiated finished')
        else:
            raise TypeError('pretrained must be a str or None')

    def forward(self, x):
        """Forward function."""
        x = x  # torch.Size([2, 3, 640, 640])

        x = self.patch_embed(x)  # torch.Size([2, 96, 160, 160])
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')  # torch.Size([2, 768, 20, 20])
                x_out = norm_layer(x_out)
                out_x = x_out
                outs.append(out_x)

        x = self.conv(x)  # torch.Size([72, 2048, 7, 7])
